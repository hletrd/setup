...
if [ "$is_macos" = "true" ]; then
  # macOS - install Powerline fonts via Homebrew cask
...
if [ -f "$claude_rules_src" ]; then
  mkdir -p "$HOME/.claude"
  cp "$claude_rules_src" "$HOME/.claude/CLAUDE.md"
else
...
if [ -f "$codex_rules_src" ]; then
  mkdir -p "$HOME/.codex"
  cp "$codex_rules_src" "$HOME/.codex/AGENTS.md"
else
...
if [ -f "$opencode_rules_src" ]; then
  mkdir -p "$HOME/.config/opencode"
  cp "$opencode_rules_src" "$HOME/.config/opencode/AGENTS.md"
else
