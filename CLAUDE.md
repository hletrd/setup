...
- **Skills (replacing MCP servers)**: `agent-browser` (9 subskills: core, config, debug, interact, network, query, state, visual, wait), `fetch`, `filesystem`, `git`, `github`, `korean-naturalizer`, `playwright`, `review-plan-fix`, `writing-fixer`, `queue-submit` (sourced from `configs/claude/skills/` and installed into `~/.claude/skills/`, `~/.codex/skills/`, `~/.config/opencode/skills/`)
- **Codex-loop worker skills** (`/queue-loop` + `/queue-submit`): `queue-loop` is a **Codex** skill (`configs/macmini-setup/common/.codex/skills/queue-loop/` in the `nas-ops` repo) — one invocation → a never-ending Codex turn that every 5 min polls `~/codex-queue` (or a passed dir) for `*.task`/`*.md`, runs the oldest to completion (task text names its own repo), archives to `done/`, forever; survives usage-limit windows since it stays in a running turn. `queue-submit` (`configs/claude/skills/queue-submit/`, portable `name`+`description` frontmatter) is the **injection** side — invoked from Claude Code (or any agent) it writes a time-sortable `<id>.task` into the same `~/codex-queue` for the running worker to pick up. Both use `find -maxdepth 1` not shell globs (zsh no-match safety) and pass Codex `quick_validate.py`. Feed the worker with `/queue-submit` or by dropping a `*.task` file into the queue dir.
