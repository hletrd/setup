...
- **Skills (replacing MCP servers)**: `agent-browser` (9 subskills: core, config, debug, interact, network, query, state, visual, wait), `fetch`, `filesystem`, `git`, `github`, `korean-naturalizer`, `playwright`, `review-plan-fix`, `writing-fixer`, `queue-loop`, `queue-submit` (sourced from `configs/claude/skills/` and installed into `~/.claude/skills/`, `~/.codex/skills/`, `~/.config/opencode/skills/`)
- **Codex-loop worker skills** (`/queue-loop` + `/queue-submit`, both now in `configs/claude/skills/` here → installer mirrors each to `~/.claude/skills`, `~/.codex/skills`, `~/.config/opencode/skills`): `queue-loop` is the **worker** — one invocation → a never-ending Codex turn that every 5 min polls `~/codex-queue` (or a passed dir) for `*.task`/`*.md`, runs the oldest to completion (task text names its own repo), archives to `done/`, forever; survives usage-limit windows since it stays in a running turn (spend cap gates only NEW turns). `queue-submit` is the **injection** side — invoked from Claude Code (or any agent) it writes a time-sortable `<id>.task` into the same `~/codex-queue` for the running worker to pick up. Both have portable `name`+`description` frontmatter (valid in Claude/Codex/opencode), use `find -maxdepth 1` not shell globs (zsh no-match safety), and pass Codex `quick_validate.py`. Feed the worker with `/queue-submit` or by dropping a `*.task` file into the queue dir. (Also backed up in the **`nas-ops`** repo: `queue-loop` under `configs/macmini-setup/common/.codex/skills/`, `queue-submit` under `.claude/skills/`.)
