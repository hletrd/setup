...
The following rules are user-level overrides. They apply on top of the OMX harness above and to all projects unless an explicit project-level instruction says otherwise.
...
- Use available tools to find current, accurate information before responding:
  ...
  - Official documentation via `WebFetch` / `curl` for authoritative API references
  - The `researcher` agent / `find-docs` skill / Context Hub when documentation accuracy matters
...
- Do NOT add `Co-Authored-By` lines to commit messages. Never attribute Codex/Claude/any AI as author or co-author.
...
- When a project does NOT have a project-level `AGENTS.md` or `CLAUDE.md`, check for a `.context/` directory at the project root.
...
- Read these files at the start of a session before doing any work, as they serve the same purpose as `AGENTS.md` for providing project-specific instructions and context.
...
- That repo contains `CLAUDE.md`, `AGENTS.md`, and a `docs/knowledge-graph/` directory with comprehensive modular documentation covering:
  ...
  - Mac mini #0 (172.30.61.1), Mac mini #1 (172.30.62.1), and Mac mini #3 (100.115.57.92 Tailscale)
