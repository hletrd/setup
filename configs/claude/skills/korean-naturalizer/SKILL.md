...
섹션 1~10이 잡지 못하는 AI 특유의 글쓰기 패턴을 제거한다. 이 섹션은 `humanizer` 스킬의 "Signs of AI writing" 분류와 삭제 우선 원칙을 한국어 산문에 맞게 옮긴 것이다. 각 패턴마다 **먼저 삭제가 가능한지**부터 따진다. 삭제로 의미가 상하지 않으면 삭제한다.
...
9. **원뜻이 보존됐는가?** 구체 수치·고유명사·인과 관계가 원문과 일치하는가? (의미 변경 금지)
